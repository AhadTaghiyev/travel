...
export const CreateAirwaySchema = Yup.object().shape({
...
export const CreateAirwayModal = () => {
  const { onClose, isOpen, type, setModalSuccess } = useModal();
  const { t } = useTranslation();

  const isModalOpen = isOpen && type === "createAirway";
  ...
  const onSubmit = async (
    ...
    const promise = apiService
      .post(`/Airways/Create`, values)
      .then((response) => {
        if (response.status === 200) {
          toast.success(t("Airway Created"));
  ...
  return (
    <Modal title={t("Yeni Havayolu Yarat")} onModalClose={onClose}>
      <Formik
        onSubmit={onSubmit}
        initialValues={initialValues}
        validationSchema={CreateAirwaySchema}
      >
        {({
          values,
          errors,
