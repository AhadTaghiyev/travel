...
const UpdateMeal = () => {
  const [income, setIncome] = useState<IMealModel>();
  ...
  async function getData(id: string) {
    const response = await apiService.get(`/Dinings/Get/${id}`);
  ...
  return (
    <div className="mx-1 p-4 bg-white shadow-md min-h-[500px]">
      ...
      {loading && <Loading />}
      {!loading && income && (
        <MealForm formType="Edit" initialValues={income} onSubmit={onSubmit} />
      )}
    </div>
  );
};
