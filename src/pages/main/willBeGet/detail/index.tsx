...
export default function index() {
    ...
    useEffect(()=> {
        ...
        const fetchData = async () => {
            try {
                const response = await apiService.get(`/Report/WillBeGetDetail?page=${id}`);
  ...
  return (
    <div style={{ height: '65vh', width: '100%' }}>
        {
            data.map((elem, index)=> (
                <SimpleTable key={index} header={elem.header} properties={elem.properties} values={elem.values}/>
            ))
        }
    </div>
  )
