...
export default function CreatePopup() {
    ...
    const getPlaneTickets = async () => {
        const planeTicketsFromApi = await apiService.get(
            "PlaneTicket/GetAll/1"
        );
        dispatch({
    ...
    const getTourPackages = async () => {
        const tourPackagesFromApi = await apiService.get(
            "tourPackage/GetAll/1"
        );
        dispatch({
    ...
    useEffect(()=>{
        (async()=>{
            ...
            try {
                const res = await apiService.get(`Refund/GetById/${id}`)
                setRefund(res.data.data)
                initialValues = {
                    ...initialValues, 
                    planeTicketId: res.data.data?.planeTicket?.id ?? null,
                    cooperativeTicketId : res.data.data?.cooperativeTicket?.id ?? null, 
                    individualTourId: res.data.data?.individualTour?.id ?? null,
                    tourPackageId:res.data.data?.tourPackage?.id ?? null,
                    depositId: res.data.data?.deposit?.id?? null, 
                    otherServiceTicketId: res.data.data?.otherServiceTicket?.id ?? null,
                    amount:res.data.data?.amount ?? 0,
                    paidToCustomer: res.data.data?.paidToCustomer??0,
                    forfeit: res.data.data?.forfeit?? 0,
                }

                // set the selected option and its  // statik yazmali oldum optimali aglima gelmedi
                 if(initialValues.planeTicketId) setOption(options[0])
                 else if(initialValues.cooperativeTicketId) setOption(options[1])
                 else if(initialValues.individualTourId) setOption(options[2])
                 else if(initialValues.tourPackageId) setOption(options[3])
                 else if(initialValues.depositId) setOption(options[4])
                 else if(initialValues.otherServiceTicketId) setOption(options[5])
            ...
            <Formik
                ...
                try {
                    const res = await apiService.put(
                        `/Refund/UpdateRefund/${id}`,
                        {   ...values, 
                            amount: state?.[option.stateName].find(x=> x.id === values[option.name])?.paidAmount,
                            forfeit: state?.[option.stateName].find(x=> x.id === values[option.name])?.paidAmount - values.paidToCustomer
                        }
                    );
                    if (res?.status == 200) {
                             ...
                             >
                                 ...
                                 <Box>
                                     ...
                                     <TextField
                                         ...
                                         style={textStyling}
                                         name={`amount`}
                                         value={state?.[option.stateName].find(x=> x.id === props.values[option.name])?.paidAmount}
                                  ...
                                  <Box>
                                     ...
                                     <TextField
                                         ...
                                         style={textStyling}
                                         name={`forfeit`}
                                         value={state?.[option.stateName].find(x=> x.id === props.values[option.name])?.paidAmount - props.values.paidToCustomer}
                                         onChange={props.handleChange}
                                         error={
                                             !!props.errors &&
                                             !!props.errors?.forfeit
                                         }
                                         helperText={
                                             props.errors &&
                                             props.errors?.forfeit
