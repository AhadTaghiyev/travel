  ...
  useEffect(() => {
    try {
      const fetchdata = async () => {
        const res = await apiService.get(`/PlaneTickets/Get/${id!}`);
        for (const key in initialValues) {
          initialValues[key] = res.data.data[key];
        }
        initialValues.invoiceDirections = res.data.data.invoiceDirections || [
          invoiceDirection,
        ];
          ...
          onSubmit={async (values, { setErrors }) => {
            try {
              const res = await apiService.put(
                `/PlaneTickets/UpdatePlaneTickets/${id}`,
                values
              );
              if (res?.status == 200) {
              ...
              <Container maxWidth="xl">
                <Grid container spacing={2} style={{ marginBottom: "70px" }}>
                  <Grid item md={2}>
                    ...
                    <Autocomplete
                      value={personal.find(
                        (per) => per.id === props.values.personalId
                      )}
                    ...
                    <Autocomplete
                      value={suppliers.find(
                        (sup) => sup.id === props.values.supplierId
                      )}
                      onChange={(event, newValue) => {
                        props.setFieldValue(
                          "supplierId",
                    ...
                    <>
                      ...
                      {props.values?.invoiceDirections?.map((elem, key) => (
                        <Fragment key={key}>
                          ...
                          <TextField
                            ...
                            size="small"
                          />
                          {props.errors?.invoiceDirections &&
                            props.touched?.invoiceDirections && (
                              <>
                                <FormHelperText sx={{ color: "red" }}>
                                  {
                                    props.errors.invoiceDirections[key]
                                      ?.direction
                                  }
                                </FormHelperText>
                              </>
                      ...
                      <Button
                        onClick={() => {
                          ...
                          const updatedInvoiceDirections = [
                  ...
                  <Grid item md={2}>
                    ...
                    >
                      {airways?.map((aw: any, i: number) => (
                        <MenuItem key={i} value={aw.id} style={textStyling}>
                          {aw.name}
                        </MenuItem>
                      ))}
                    </Select>
                    ...
                    {props.values?.isCustomerPaid && (
                      <>
                        ...
                        <TextField
                          ...
                          style={textStyling}
                          value={
                            props.values?.commonPrice! -
                            props.values?.paidAmount!
                          }
                        />
                        {/* <InputLabel
                            id="demo-simple-select-label"
