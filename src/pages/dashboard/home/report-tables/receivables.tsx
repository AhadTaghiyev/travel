...
interface IReciveablesReportProps {
  selectedYear: string;
}
...
const ReciveablesReport = ({ selectedYear }: IReciveablesReportProps) => {
  ...
  const getData = async () => {
    ...
    await apiService
      .get(`/Reports/ReciveAblesReport/1?${searchParams.toString()}`)
      .then(({ data }) => {
        setData(data.items ?? []);
      })
      .catch((err) => {
                ...
                <TableRow key={row.id}>
                  ...
                  {columns.map((column) => (
                    <TableCell key={column.name} className={"py-1.5"}>
                      {row?.[column.name] === "Founder/Debt" || row?.[column.name] === "Founder" ? t("Others") : row?.[column.name]}
                    </TableCell>
                  ))}
                </TableRow>
              ))}
...
export default ReciveablesReport;
