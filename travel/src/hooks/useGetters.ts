...
export const useGetters = () => {
    ...
    const getServiceManagers = useCallback(
        async () => {
            const serviceManagersFromApi = await apiService.get('ServiceManager/GetAll/1')
        dispatch({type:reducerActions.serviceManagers, payload:serviceManagersFromApi.data.items}) 
    },[]) 
