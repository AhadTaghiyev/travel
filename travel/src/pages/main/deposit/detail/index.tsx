...
export default function CreatePopup() {
    ...
    useEffect(()=>{
        (async()=>{
            setIsLoading(true)
            try {
                const depositRes = await apiService.get(`Deposit/GetById/${id}`)
                setDeposit(depositRes.data.data)
                initialValues = {
                    ...initialValues,
                    ...depositRes.data.data,
                    customerId: depositRes.data.data.customer?.id
                }
            } catch (err) {
                console.log("🚀 ~ file: index.tsx:76 ~ err:", err)
            }
            finally{
                setIsLoading(false)
            }
    ...
    useEffect(() => {
        (async () => {
            ...
            try {
                const customersRes = await apiService.get("Customer/GetAll/1");
                ...
                onSubmit={async (values, { setErrors }) => {
                    try {
                        const res = await apiService.put(
                            `/Deposit/UpdateDeposit/${id}`,
                            values
                        );
                        if (res?.status == 200) {
                            toast.success("Depozit uğurla yeniləndi!");
