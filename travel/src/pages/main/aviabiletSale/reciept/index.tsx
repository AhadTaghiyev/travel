  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await apiService.get(`/PlaneTicket/GetById/${guid}`);
...
tickets.map(ticket => {
  ...
  const formattedDate = new Intl.DateTimeFormat('en-US').format(new Date(ticket.createdAt));
  ...
  return (
      ...
      />
        ...
        <Button  variant="contained" color='inherit' sx={{mr: 2 ,mb:1}} onClick={()=> navigate(-1)}>Geri qayıt</Button>
    </Container>
  )
}
