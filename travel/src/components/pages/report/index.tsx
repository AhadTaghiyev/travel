...
export default function Index({headers, api,service}: IReportModel) {
    ...
    useEffect(() => {
        async function fetchCurrentUser() {
            try {
                const res = await apiService.get('User/GetCurrentUser');
                if (res && res.data) {
  ...
  return (
    <Container   maxWidth='xl' sx={{mb: 5,backgroundColor:"white"}}>
        <Grid container spacing={3} sx={{ mb: 2, width: '100%',borderTop:"1px solid #d8d3d3" ,mt:1}}>
          <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'end'}}>
            ...
            <Button variant="text" color='inherit' sx={{ml: 4, fontSize: '12px', lineHeight: '16px'}}> <AiOutlineMail style={{marginRight: '8px'}}/>  Send mail</Button>
            <Button onClick={handlePrint} variant="text" color='inherit' sx={{ml: 4, fontSize: '12px', lineHeight: '16px'}}><FiDownload style={{marginRight: '8px'}}/> Print</Button>
          </Grid>
          <Grid container sx={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
            ...
            <Grid item xs={5}>
                <Typography variant="h4" gutterBottom align='right'>{currentUser.companyName}</Typography>
                <Typography gutterBottom align='right'>Email: {currentUser.companyEmail}  |  Tel: {currentUser.companyPhone}</Typography>
            </Grid>
          </Grid>
           <Container maxWidth='xl' sx={{mb: 5,mt:2}}>
            <Grid  container spacing={3}>
                <Grid sx={{backgroundColor:"white"}} item xs={6}>
                    <SimpleTable header='Müştəri məlumatları' properties={customerProperties} values={customer}/>
                </Grid>
            </Grid>
          </Container> 
